...
class RBFnet:
    ...
    def initialiseWeights(self):
        self.inputWeights  = np.ones((self.neurons, self.n_input))*5
    ...
    def trainLM(self, notappend=False):
        ...
        for i, error in enumerate(self.errors):
            ...
            # print("\n")
            # print(np.exp(-self.rbfInput[:, i]**2).dot(self.rbfInput[:, i]))
            self.J21[i, :] = (error * 2 * self.input[0, i] * self.outputWeights * ((np.exp(-self.rbfInput[:, i]**2).dot(self.rbfInput[:, i])))).flatten()
            # Problem with missing second dimension on np.exp part!

            self.J22[i, :] = (error * 2 * self.input[1, i] * self.outputWeights * ((np.exp(-self.rbfInput[:, i]**2).dot(self.rbfInput[:, i])))).flatten()
...
dis, cents, closest, colors = k_means(alpha, beta, 300)
...
while test_net.sum_sq_errors > 1e-6:
    ...
    if i_count == 70 or test_net.learningRate < 1e-40:
        break
    i_count += 1
